...
class DetectionValidator(BaseValidator):
    ...
    def get_desc(self):
        ...
        return ('%22s' + '%11s' * 8) % ('Area', 'Instances', 'tp', 'fp' ,'fn','Box(P', 'R', 'mAP50', 'mAP50-95)')
    ...
    def print_results(self):
        ...
        pf = '%22s' + '%11i' * 4 +'%11.3g' * (len(self.metrics.keys)-2)  # print format
        tp,fp,p,r,map50,map = self.metrics.mean_results()
        fn = self.nt_per_class.sum() - tp
        LOGGER.info(pf % ('all', self.nt_per_class.sum(), tp,fp,fn,p,r,map50,map))
