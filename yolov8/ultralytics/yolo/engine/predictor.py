...
class BasePredictor:
    ...
    def stream_inference(self, source=None, model=None):
        ...
        # Record second classifier's inference time
        if self.args.classifier:
            profilers = (ops.Profile(), ops.Profile(), ops.Profile(), ops.Profile())
        ...
        for batch in self.dataset:
            ...
            # Postprocess
            with profilers[2]:
                self.results = self.postprocess(preds, im, im0s, profilers)
        ...
        if self.args.verbose and self.seen:
            ...
            if self.args.classifier:
                t = (t[0], t[1], abs(t[3]-t[2]), t[3])
                LOGGER.info(f'Speed: %.1fms preprocess, %.1fms inference, %.1fms postprocess, %.1fms classifier per image at shape '
                        f'{(1, 3, *self.imgsz)}' % t)
            else:
